...
export const ListarUsuario = ({
  ...
  const [abrirModalDocumento, setAbrirModalDocumento] = useState(false);
  const [datosModal, setDatosModal] = useState(null);
  const [datosModalDocumento, setDatosModalDocumento] = useState(null);
  ...
  const mostrarModalDocumento = (record) => {
    setAbrirModalDocumento(true);
    setDatosModalDocumento(record);
  };
  const handleBuscar = () => {
    traerDatos(pagination, buscador, usuarioActivo);
  };
      ...
      render: (text, record) => (
        <span>
          ...
          <span className="gx-link">
            {" "}
            <i
              onClick={() => mostrarModalDocumento(record)}
  ...
  return (
    <div>
      <Card
        title={
          <Row>
            ...
            >
              <h2 style={{ margin: 0, padding: 0 }}>Conductores</h2>
            </Col>
            {/*//*Switch inactivos
                         <Col
                            lg={12}
      ...
      {abrirModal ?
        ...
        : null}
      {abrirModalDocumento ?
        <ModalDocumento datosModal={datosModalDocumento} abrirModal={abrirModalDocumento} setAbrirModal={setAbrirModalDocumento} />
        : null}
    </div>
  );
};
